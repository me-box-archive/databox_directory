...
RUN apt-get update \
  ...
  && curl --insecure -SLO "https://nodejs.org/dist/v$NODE_VERSION/node-v$NODE_VERSION-linux-x64.tar.xz" \
...
#DATABOX config
LABEL databox.type="directory"


#mysql config
ENV MYSQL_ROOT_PASSWORD databox
ENV MYSQL_DATABASE databox_directory
ENV MYSQL_USER databox
ENV MYSQL_PASSWORD databox

ADD database/create.sql /docker-entrypoint-initdb.d/
ADD database/seed_content.sql /docker-entrypoint-initdb.d/
